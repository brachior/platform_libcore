...
public class MethodHandles {
    ...
    public static Lookup lookup() {
        return new Lookup(Lookup.PUBLIC);
    }
    ...
    public static final class Lookup {
        ...
        private final int lookupMode;

        Lookup(int lookupMode) {
         ...
         */
        private static final Lookup PUBLIC_LOOKUP = new Lookup(PUBLIC);
        ...
        public MethodHandle findStatic(Class<?> refc, String name, MethodType type)
            ...
            //FIXME implements security checks
            return MethodHandle.createMethodMH(MethodHandle.REF_invokeStatic, refc, name, type);
        }
        ...
        public MethodHandle findVirtual(Class<?> refc, String name, MethodType type)
            ...
            int refKind = (refc.isInterface()) ? MethodHandle.REF_invokeInterface : MethodHandle.REF_invokeVirtual;
            return MethodHandle.createMethodMH(refKind, refc, name, type);
        }
         ...
         * <p/>
         * The returned method handle will have
         * {@linkplain MethodHandle 0x0080}) is set.
         *
         * @param refc MethodHandle.REF_invokeVirtuallass or interface from which the method is accessed
        ...
        public MethodHandle findSpecial(Class<?> refc, String name, MethodType type, Class<?> specialCaller)
                throws NoSuchMethodException, IllegalAccessException {
            // FIXME implements security checks
            return MethodHandle.createMethodMH(MethodHandle.REF_invokeSpecial, refc, name, type);
        }
        ...
        public MethodHandle findGetter(Class<?> refc, String name, Class<?> type)
            ...
            // FIXME implements security checks
            return MethodHandle.createFieldMH(MethodHandle.REF_getField, refc, name, type);
        }
        ...
        public MethodHandle findSetter(Class<?> refc, String name, Class<?> type)
                throws NoSuchFieldException, IllegalAccessException {
            // FIXME implements security checks
            return MethodHandle.createFieldMH(MethodHandle.REF_putField, refc, name, type);
        }
        ...
        public MethodHandle findStaticGetter(Class<?> refc, String name, Class<?> type)
                throws NoSuchFieldException, IllegalAccessException {
            // FIXME implements security checks
            return MethodHandle.createFieldMH(MethodHandle.REF_getStatic, refc, name, type);
        }
        ...
        public MethodHandle findStaticSetter(Class<?> refc, String name, Class<?> type)
                throws NoSuchFieldException, IllegalAccessException {
            // FIXME implements security checks
            return MethodHandle.createFieldMH(MethodHandle.REF_putStatic, refc, name, type);
        }
        ...
        public MethodHandle unreflect(Method m) throws IllegalAccessException {
            // FIXME implements security checks
            long info = MethodHandle.extractMethodVMInfo(m);
            MethodType methodType = MethodType.unsafeMethodType(m.getReturnType(), m.getParameterTypes());
            int slot = (int) (info & 0xFFFFFFFF);
            int kind = (int) (info >>> 32);
            return new MethodHandle(kind, slot, m.getDeclaringClass(), methodType);
        }
         ...
         * the method's variable arity modifier bit ({@code 0x0080}) is set.
         *
         * @param m int slothe reflected method
         *          Slotam specialCaller the class nominally calling the method
         * @return a method handle which can invoke the r*                                or if the method'sr bit
         *         is set and {@code asVarargsCollector} fails
         * @throws NullPointerException if any argument is null
         */
        public MethodHandle unreflectSpecial(Method m, Class<?> specialCaller) throws IllegalAccessException {
            ...
            int slot = (int) (info & 0xFFFFFFFF);
            // int kind = (int) (info >>> 32);
            return new MethodHandle(MethodHandle.REF_invokeSpecial, slot, m.getDeclaringClass(), methodType);
        }
         ...
         * @return a meint slotle which can invoke the reflectSlotonstructor
         * @throws IllegalAccessException if access checking fails
         *                                is set and {@code asVarargsCollector} fails
        ...
        public MethodHandle unreflectGetter(Field f) throws IllegalAccessException {
            ...
            int refKind = (isStatic) ? MethodHandle.REF_getStatic : MethodHandle.REF_getField;
            long info = MethodHandle.extractFieldVMInfo(f, refKind);
            int slot = (int) (info & 0xFFFFFFFF);
            int kind = (int) (info >>> 32);
            MethodType methodType = (isStatic) ? MethodType.methodType(f.getType())
                    : MethodType.unsafeMethodType(f.getType(), f.getDeclaringClass());
            return new MethodHandle(kind, slot, f.getDeclaringClass(), methodType);
        }
        ...
        public MethodHandle unreflectSetter(Field f) throws IllegalAccessException {
            // FIXME implements security checks
            boolean isStatic = Modifier.isStatic(f.getModifiers());
            int refKind = (isStatic) ? MethodHandle.REF_putStatic : MethodHandle.REF_putField;
            long info = MethodHandle.extractFieldVMInfo(f, refKind);
            int slot = (int) (info & 0xFFFFFFFF);
            int kind = (int) (info >>> 32);
            MethodType methodType = (isStatic) ? MethodType.unsafeMethodType(void.class, f.getType())
                    : MethodType.unsafeMethodType(void.class, f.getDeclaringClass(), f.getType());
            return new MethodHandle(kind, slot, f.getDeclaringClass(), methodType);
        }
    }
